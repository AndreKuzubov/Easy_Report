...
[DllImport("kernel32.dll", CharSet = CharSet.Auto, SetLastError = true)]
    ...
    {
        this.SelectedRtf = WFormsAppWordExport.RTFHelpener.makeMyScript(idScript);
    }
    public int getIdSript(int iCharIndex)
    {
        return new WFormsAppWordExport.RTFHelpener(Rtf).getIdSript(iCharIndex);
    }

    public void delScript(int idScript)
    {
        WFormsAppWordExport.RTFHelpener h= new WFormsAppWordExport.RTFHelpener(Rtf);
        h.delScript(idScript);
        Rtf = h.Rtf;
    }

    public int getRtfIndexByTextIndex(int iCharIndex)
    {
        return new WFormsAppWordExport.RTFHelpener(Rtf).getRtfIndexByTextIndex(iCharIndex);
    }
    #endregion
